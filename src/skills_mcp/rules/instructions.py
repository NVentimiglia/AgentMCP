...
def render_mcp_seed_text(
    *,
    ...
    """MCP ``instructions`` string: preamble + optional AGENT.md content.

    If ``agent_md_content`` is provided (from ``.agents/AGENT.md``), it is
    appended after the preamble under a ``## Rules`` heading.
    """
    if agent_md_content:
        seed = _BASE + "\n## Rules\n\n" + agent_md_content
    else:
    ...
    if budget < 1:
        return seed[:limit]
    return seed[:budget].rstrip() + suffix
